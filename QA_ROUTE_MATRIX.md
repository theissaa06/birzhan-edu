...
|---|---|---|---|---|---|---|---|
| `/register` | Guest | `POST /api/auth/register`, OAuth providers | form, Turnstile, four OAuth buttons, errors | 390×844 passed; no overflow; menu passed | four-provider readiness UI test + lint/typecheck/test/build passed | form + four buttons passed; Google enabled with real provider consent screen | pending production release; Apple/Telegram/VK await provider credentials |
| `/login` | Guest | login, reactivate, OAuth exchange | password, deactivated recovery, one-use OAuth code | desktop passed; mobile host cold-start timed out on preview | atomic exchange/replay + provider readiness tests passed | Google reached account selection and consent; final callback inspection blocked by Browser URL policy | pending production release; Apple/Telegram/VK await provider credentials |
